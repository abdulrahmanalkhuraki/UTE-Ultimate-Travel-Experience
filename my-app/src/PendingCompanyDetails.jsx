...
const GlassCard = ({ title, icon: TitleIcon, children, borderColor = "border-[#D4AF37]/20" }) => (
  <div className={`bg-[#18181A]/80 backdrop-blur-md border ${borderColor} rounded-2xl p-5 shadow-[0_8px_30px_rgb(0,0,0,0.15)] hover:shadow-[0_8px_30px_rgb(0,0,0,0.25)] transition-all duration-300`}>
    <div className="flex items-center gap-3 mb-4 pb-3 border-b border-[#333]/50">
    ...
    <div className="p-8 space-y-6 font-sans animate-in fade-in zoom-in-95 duration-300">
      ...
      <div className="flex items-center justify-between border-b border-[#333] pb-4">
        <div className="flex items-center gap-4">
          ...
          <div className="w-12 h-12 rounded-xl overflow-hidden border-2 border-[#D4AF37]/50 shadow-lg">
          ...
          <div>
            <h2 className="text-2xl font-bold text-white">{company?.name || "Alpine Adventures"}</h2>
            <p className="text-sm text-[#F4A261] flex items-center gap-1.5 mt-0.5">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        ...
        <div className="space-y-6">
          <h2 className="text-lg font-bold text-[#91B3FA] px-2 flex items-center gap-2">
            <Building2 className="w-5 h-5" /> Company Details
          </h2>
          
          <GlassCard title="Identity & Description" icon={Building2} borderColor="border-[#91B3FA]/30">
          ...
          <GlassCard title="Contact & Location" icon={MapPin} borderColor="border-[#91B3FA]/30">
          ...
          <GlassCard title="Legal & Financial" icon={FileText} borderColor="border-[#91B3FA]/30">
            ...
            {/* صورة السجل */}
            <div className="mt-4 p-3 bg-[#121212] rounded-xl border border-[#333]">
              <span className="text-xs text-gray-400 mb-2 flex items-center gap-1.5"><ImageIcon className="w-3.5 h-3.5" /> Tourism Record Image</span>
              {company?.tourismLicenseImage ? (
                <div className="w-full h-24 rounded-lg overflow-hidden border border-[#333]">
                  <img src={company.tourismLicenseImage} alt="Tourism Record" className="w-full h-full object-cover" />
                </div>
              ) : (
                <div className="w-full h-24 border-2 border-dashed border-[#444] rounded-lg flex items-center justify-center text-gray-600 bg-[#18181A]">
        ...
        <div className="space-y-6">
          <h2 className="text-lg font-bold text-[#F4A261] px-2 flex items-center gap-2">
            <User className="w-5 h-5" /> Owner Details
          </h2>

          <GlassCard title="Personal Information" icon={User} borderColor="border-[#F4A261]/30">
          ...
          <GlassCard title="Contact & Financial" icon={Phone} borderColor="border-[#F4A261]/30">
          ...
          <GlassCard title="Identity Document" icon={Fingerprint} borderColor="border-[#F4A261]/30">
             {/* صورة الهوية */}
             <div className="p-3 bg-[#121212] rounded-xl border border-[#333]">
              <span className="text-xs text-gray-400 mb-2 flex items-center gap-1.5"><ImageIcon className="w-3.5 h-3.5" /> ID Image</span>
      ...
      <div className="flex items-center gap-4 pt-6 border-t border-[#333] mt-8">
        <button 
          onClick={onReject}
          className="flex-1 flex justify-center items-center gap-2 bg-[#2A2A2D] text-red-400 py-3.5 rounded-xl font-bold hover:bg-red-500/10 hover:border-red-500/30 border border-transparent transition duration-300"
        >
          <XCircle className="w-5 h-5" /> Reject Application
        </button>
        <button 
          onClick={onApprove}
          className="flex-1 flex justify-center items-center gap-2 bg-[#91B3FA] text-black py-3.5 rounded-xl font-bold hover:bg-[#7fa1e8] shadow-[0_0_20px_rgba(145,179,250,0.2)] transition duration-300"
        >
