...
{
    /// <summary>Update-time rules for a tour guide. Mirrors the create rules.</summary>
    public sealed class TouristGuideUpdateValidator : AbstractValidator<TouristGuideUpdateRequest>
    {
        public TouristGuideUpdateValidator()
        {
            ...
            // Each image is required: either upload a new file or keep the existing URL.
            RuleFor(x => x)
                .Must(x => x.ProfileImage != null || !string.IsNullOrWhiteSpace(x.ProfileImageUrl))
                .WithMessage("Profile image is required");

            RuleFor(x => x)
                .Must(x => x.IdCardImage != null || !string.IsNullOrWhiteSpace(x.IdCardUrl))
                .WithMessage("National ID image is required");

            RuleFor(x => x)
                .Must(x => x.PassportImage != null || !string.IsNullOrWhiteSpace(x.PassportScanUrl))
                .WithMessage("Passport image is required");
        }
    }
}
