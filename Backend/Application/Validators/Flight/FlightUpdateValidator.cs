...
{
    public class FlightUpdateValidator : AbstractValidator<FlightUpdateRequest>
    {
        public FlightUpdateValidator()
        {
            RuleFor(x => x.Id)
                .GreaterThan(0).WithMessage("Valid flight ID is required for update");
            ...
            RuleFor(x => x)
                .Must(x => x.Departure == default || x.Arrival == default || (x.Arrival - x.Departure).TotalHours <= 24)
                .WithMessage("Flight duration cannot exceed 24 hours")
                .Must(x => x.Departure == default || x.Arrival == default || (x.Arrival - x.Departure).TotalMinutes >= 30)
